...
int success=0; //epituximenes paraggelies
int failure=0; //akurwmenes paraggelies
...
void *order(void *x){
	...
	while(Noven<pizza){
		rc=pthread_cond_wait(&cond_oven,&lockoven);
	}
	Noven=Noven - pizza;
	...
	while(Npack==0){
		rc=pthread_cond_wait(&cond_pack,&lockpack);
	}
	Npack--;
	...
	while(Ndeliverer==0){
		rc=pthread_cond_wait(&cond_del,&lockdel);
	}
	Ndeliverer--;
